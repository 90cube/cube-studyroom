...
const explanations: ExplanationEntry[] = [
  ...
  "LoRA를 얹기 전, 맨몸 SD1.5가 'platypus(오리너구리)'를 어떻게 그리는지 먼저 봐. 시드 5개로 5장 뽑아서 한 줄에 깔아. 여기서 cross_attention_kwargs의 scale=0.0은 'LoRA 효과 0%' — 즉 아직 LoRA가 없으니 순수 베이스 모델 결과야. 나중과 비교할 기준선을 만드는 거지.",
  // 3 — load_lora_weights
  {
  ...
  // 5 — lora_scale sweep
  "LoRA를 '얼마나 세게' 적용할지 손잡이를 0.0부터 1.4까지 쭉 돌려가며 한 장씩 뽑아 나란히 깔아. scale=0이면 베이스 모델 그대로, 1.0이면 학습한 그대로, 1.4면 과하게 밀어붙인 거야. 이 한 줄짜리 스윕으로 'LoRA는 켜고 끄는 스위치가 아니라 세기 조절 다이얼'이라는 걸 눈으로 확인해 — 보통 0.6~1.0 사이가 제일 자연스러워. (실무에선 set_adapters로 화풍 LoRA와 캐릭터 LoRA를 각자 다른 scale로 섞어 새 조합을 만들기도 해.)",
