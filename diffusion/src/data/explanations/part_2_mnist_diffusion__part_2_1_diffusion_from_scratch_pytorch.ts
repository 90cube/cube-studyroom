...
const explanations: ExplanationEntry[] = [
  ...
  {
    text: "노이즈 일정표를 짜. 1000단계 동안 매 단계 얼마나 노이즈를 섞을지(beta)를 아주 작은 값에서 0.01까지 선형으로 깔고, alpha=1−beta, 거기서 '누적 곱 alpha_bar'를 구해. 이 alpha_bar 하나면 0단계 원본에서 임의의 t단계 노이즈 이미지로 한 방에 점프할 수 있어 — 매 단계 일일이 노이즈 안 더해도 되는 게 DDPM의 핵심 트릭이야. 곡선을 그려보면 1에서 0으로 미끄러지며 원본이 사라지는 게 보여. (보라색 줄에 마우스 올리면 줄별 풀이가 떠.)",
    lines: {
      1: "T=1000. 전체 노이즈 단계 수 — 원본을 1000칸에 걸쳐 노이즈로 녹였다 되돌린다.",
      2: "betas = 단계별 노이즈 양. 1e-4에서 0.01까지 선형으로 증가. 처음엔 살살, 뒤로 갈수록 세게 더해.",
      3: "alphas = 1−beta. '이 단계에서 노이즈 말고 원본이 남는 비율'.",
      4: "alpha_bars = alpha들의 누적곱. t단계까지 원본이 얼마나 살아남는지 — √ᾱ·원본 + √(1−ᾱ)·노이즈 공식의 그 ᾱ야.",
    },
    diagram: {
  ...
  // 7 — MyUNet class
  {
    text: "노이즈를 걷어낼 UNet을 맨손으로 설계해. 가운데로 갈수록 해상도를 절반씩 줄이며(MaxPool) 특징을 압축하는 내리막(down) 3층, 다시 2배씩 키우며(Upsample) 복원하는 오르막(up) 3층 구조야. 핵심은 두 가지 — (1) 내리막에서 뽑은 특징을 같은 해상도 오르막에 더해주는 skip 연결(h.pop), 이게 디테일을 살려. (2) '지금 몇 단계인지(t)'와 '어떤 숫자인지(cls)'를 각각 작은 MLP로 임베딩해서 첫 블록 특징맵에 더해줘 — 모델이 시간 감각과 클래스를 동시에 갖게 하는 거야. 출력은 입력과 똑같은 28×28, 즉 '이 이미지에 낀 노이즈'의 예측이야. (보라색 줄에 마우스 올리면 줄별 풀이가 떠.)",
    lines: {
      71: "t(단계 번호)를 작은 MLP로 32차원 벡터로 바꿔 — 숫자 하나를 모델이 쓸 수 있는 '시간 신호'로 펼치는 거야.",
      72: "cls(숫자 라벨)도 같은 식으로 임베딩 — '어떤 숫자를 만들지' 조건 신호.",
      79: "첫 블록 특징맵에 시간·클래스 임베딩을 더해 주입. [:, :, None, None]은 (B,32) 벡터를 (B,32,1,1)로 펴서 H×W 전체에 브로드캐스트하는 트릭.",
      81: "내리막 특징을 h에 저장 — 나중에 같은 해상도 오르막에서 꺼내 쓸 skip 백업.",
      82: "MaxPool로 해상도 ÷2. 점점 작게 압축하며 큰 구조를 봐.",
      87: "오르막에선 먼저 Upsample로 해상도 ×2 키우고,",
      88: "방금 키운 특징에 h.pop()으로 아까 저장한 내리막 특징을 더해 — 압축하며 잃은 디테일을 되살리는 skip 연결의 핵심.",
    },
    diagram: {
  ...
  // 10 — training loop
  {
    text: "이제 본 게임 — 모델을 훈련시켜. 손실은 MSE, 옵티마이저는 AdamW(lr=5e-4). 50에폭 동안 배치마다: 원본에 무작위 t를 골라 노이즈를 한 방에 섞고(x_noised), UNet한테 라벨까지 같이 주며 '여기 낀 노이즈 맞혀봐' 시킨 뒤, 진짜 노이즈와 얼마나 틀렸는지 재서 backward로 모델을 조금씩 고쳐. zero_grad로 기울기 초기화하는 것 잊지 말고. 5에폭마다 generate_image로 숫자가 점점 또렷해지는 걸 확인해. 실전 응용으로 보면 이게 Stable Diffusion·DALL·E 같은 생성 모델의 학습 심장부랑 똑같은 구조야 — 규모(데이터·파라미터·해상도)만 키운 거지 원리는 이 루프 그대로야. (보라색 줄에 마우스 올리면 줄별 풀이가 떠.)",
    lines: {
      14: "배치 안 각 이미지마다 무작위 t를 뽑아 — 한 배치에 여러 시점이 섞여야 모델이 전 구간을 골고루 배워.",
      18: "뽑은 t의 alpha_bar를 [:,None,None,None]로 (B,1,1,1) 모양으로 펴서, 이미지마다 다른 계수를 픽셀 전체에 곱할 준비.",
      19: "x_noised = √ᾱ·원본 + √(1−ᾱ)·노이즈. 한 줄로 t단계 노이즈 이미지를 만드는 forward 공식.",
      21: "UNet에 노이즈 이미지·t·라벨을 줘서 '낀 노이즈'를 예측시켜.",
      23: "예측 노이즈 vs 진짜 노이즈의 MSE — 이 오차가 학습 신호 전부.",
      25: "backward로 오차의 기울기를 거꾸로 흘려 각 파라미터가 얼마나 틀렸는지 계산.",
      28: "step으로 한 발 고친 뒤 zero_grad로 기울기 초기화 — 안 비우면 다음 배치 기울기에 누적돼 버려.",
    },
    diagram: {
