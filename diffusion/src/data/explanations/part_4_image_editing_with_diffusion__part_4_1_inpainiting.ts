...
const explanations: ExplanationEntry[] = [
  ...
  {
    text: "인페인팅의 핵심 도구, '마스크'를 만들어. 256×256 흰 판(전부 1)을 깔고 위쪽 100줄만 0으로 칠해 — 여기가 '새로 그려낼 영역'이야. 딱 자르면 경계가 칼처럼 티 나니까 gaussian_filter로 가장자리를 부드럽게 번지게 해(sigma=5). 그리고 1에서 빼서 반대 마스크(inv_mask)도 만들어. 둘은 한 쌍이야: mask는 '원본 유지할 곳', inv_mask는 '생성으로 채울 곳'. 원본 위에 두 마스크를 반투명으로 겹쳐 그려서 어디가 어디인지 눈으로 확인해. (보라색 줄에 마우스 올리면 줄별 풀이가 떠.)",
    lines: {
      5: "전부 1인 256×256 흰 판 — 일단 '전 영역 원본 유지'로 시작.",
      6: "위쪽 100줄만 0으로 — 1=유지, 0=새로 그림. 여기 0 구역이 모델이 채울 영역이야.",
      9: "딱 자른 0/1 경계는 칼처럼 티 나니까 gaussian_filter(σ=5)로 가장자리를 0~1 사이로 부드럽게 번지게 해 — 합성 이음매를 자연스럽게.",
      13: "inv_mask = 1 − mask. mask와 정확히 반대 — '생성으로 채울 곳' 가중치. 둘이 한 쌍으로 합성에 쓰여.",
    },
    diagram: {
      title: "소프트 마스크 한 쌍",
      kind: "architecture",
  ...
  {
    text: "드디어 다시 칠하기(repaint). 핵심 아이디어 한 줄이야 — '매 스텝, 마스크 안쪽은 원본을 그대로 끼워넣고, 바깥쪽만 모델이 상상해서 채우게 한다.' 그래서 루프 안에서 두 가지가 동시에 돌아: ① 모델은 노이즈 xt를 한 칸 디노이즈하고(생성), ② 동시에 원본 사진에도 지금 시각 t에 딱 맞는 양만큼 노이즈를 입혀(add_noise) x_noised를 만들어. 그리고 둘을 마스크로 섞어: 유지할 영역(mask)엔 노이즈 입힌 원본을, 새로 그릴 영역(inv_mask)엔 모델이 만든 걸 넣어. 단계마다 같은 노이즈 레벨에서 합치니까 이음매 없이 자연스럽게 메워져. 이게 RePaint 논문(arXiv 2201.09865)이 쓰는 방식이고, 요즘 Diffusers 인페인트 파이프라인도 같은 'blended diffusion' 골격을 따라. seed를 0~9로 바꿔가며 10장 뽑아 — 같은 자리를 매번 다르게 채우는 다양성을 보는 거야. (보라색 줄에 마우스 올리면 줄별 풀이가 떠.)",
    lines: {
      18: "원본에 입힐 새 노이즈 — 매 스텝 새로 뽑아.",
      19: "x_noised = 원본을 '지금 시각 t' 수준만큼만 더럽힌 버전. 생성 중인 xt랑 노이즈 레벨을 맞춰야 섞었을 때 안 어긋나.",
      24: "모델은 생성 중인 xt를 보고 낀 노이즈를 예측 — 마스크 바깥(새로 그릴 곳)을 상상하는 쪽.",
      27: "예측으로 xt를 한 칸 디노이즈 (생성 진행).",
      28: "이 줄이 인페인팅의 심장 — 유지영역(mask)엔 노이즈 입힌 원본을, 생성영역(inv_mask)엔 모델 결과를 픽셀별로 합쳐. 같은 t 레벨에서 섞어 이음매가 없어.",
    },
    diagram: {
