...
const explanations: ExplanationEntry[] = [
  ...
  {
    text: "여기가 LDM의 동기를 보여주는 핵심 실험이야. 같은 픽셀용 U-Net에 64·128·256·512 해상도를 차례로 넣어보고, 각 경우 GPU 최대 메모리(peak)를 재서 곡선으로 그려. 결과를 보면 해상도가 2배 되면 메모리가 대략 4배씩 폭증해 — 512에서 벌써 1GB를 넘어. 픽셀 공간에서 고해상도를 직접 다루는 게 왜 비싼지가 그래프 한 장에 박혀. 그래서 LDM의 발상: '이미지를 작게 압축해놓고 거기서 확산을 돌리자.' 다음 셀부터 그 압축기(VAE)를 데려와. (보라색 줄에 마우스 올리면 줄별 풀이가 떠.)",
    lines: {
      6: "측정 전 peak 카운터를 0으로 리셋 — 이번 해상도 한 번만 깨끗이 재려고.",
      9: "그 해상도짜리 가짜 입력 텐서(RGB 3채널). 실제 이미지 대신 랜덤 노이즈로 메모리만 잰다.",
      14: "U-Net 1회 추론. no_grad라 기울기 메모리는 안 쓰고 순수 forward 비용만 측정.",
      17: "max_memory_allocated = 이번 forward 동안 찍은 GPU 메모리 최고점(bytes).",
      26: "해상도(x축) vs peak 메모리(y축) 곡선. 위로 휘는 모양 = 해상도² 비례라는 증거.",
    },
    diagram: {
  ...
  {
    text: "이미지 배치를 VAE 인코더에 통과시켜 latent로 압축해. vae.encode(...).latent_dist.sample()은 '분포에서 한 번 뽑기'야 — VAE는 점 하나가 아니라 가우시안 분포를 내놓고, 거기서 샘플링해서 (5, 4, 64, 64)짜리 latent를 얻어. 입력 이미지값은 깔끔하게 −1~1인데, 갓 뽑은 raw latent의 min/max를 찍어보면 −57~+39처럼 범위가 제멋대로 널뛰어. 이 들쭉날쭉함이 다음 셀에서 '스케일링'이 왜 필요한지의 이유가 돼. (보라색 줄 hover 풀이 참고.)",
    lines: {
      2: "입력 이미지의 min/max를 먼저 찍어봐 — 정규화돼서 깔끔하게 −1~+1 범위.",
      5: ".latent_dist는 분포(평균·분산), .sample()로 거기서 한 점 뽑기 → (5,4,64,64) latent.",
      9: "그 raw latent의 min/max는 −57~+39처럼 제멋대로 — 그래서 다음 셀에서 스케일링이 필요.",
    },
    diagram: {
  ...
  {
    text: "raw latent에 vae.config.scaling_factor(SD1.5는 약 0.18215)를 곱해 크기를 길들여. 이 상수는 학습 데이터로 잰 latent의 표준편차라서, 곱하고 나면 latent가 대략 '분산 1' 근처로 정돈돼 — 그래야 확산 노이즈 스케줄이랑 박자가 맞거든. 곱한 뒤 min/max를 다시 찍으면 −10~+7 정도로 얌전해진 게 보여. latent 4채널을 평균 내 흑백으로 띄워보면 얼굴의 흐릿한 윤곽이 비쳐 — 압축됐어도 의미는 살아있다는 증거야. (확산 학습/샘플링은 항상 이 '스케일된 latent' 위에서 도는 게 표준 규약. 보라색 줄 hover 참고.)",
    lines: {
      2: "* scaling_factor(≈0.18215) — raw latent 표준편차로 나눠 분산 ≈ 1로 정돈하는 한 줄.",
      5: "스케일 후 min/max는 −10~+7 정도 — 앞의 −57~+39보다 확 얌전해진 게 핵심.",
      7: "4채널을 평균 내 흑백 한 장으로(torch.mean dim=0). 얼굴 윤곽이 비치면 의미 보존 OK.",
    },
    diagram: {
