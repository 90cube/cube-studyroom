...
const explanations: ExplanationEntry[] = [
  ...
  // 11 — 뎁스 맵 생성
  "참조 이미지로 뎁스 맵을 만들어. depth_estimator를 돌려 거리 정보를 흑백으로 받고, 1채널을 3번 쌓아 RGB로 만든 뒤 (512,786)으로 맞춰. 가까운 곳은 밝게/먼 곳은 어둡게 표현된 이 맵이 장면의 3D 구조(앞뒤·깊이)를 모델에 알려줄 거야.",
  // 12 — 포즈 맵 생성
  "같은 참조 이미지로 포즈 맵도 만들어(hand_and_face=True). 뎁스 맵과 크기를 똑같이 (512,786)으로 맞춰야 두 조건이 픽셀 단위로 정렬돼 함께 들어갈 수 있어. 이제 '깊이 지도'와 '골격 지도' 두 장이 같은 좌표계에 준비됐어.",
  // 13 — 두 조건 맵 + 원본 시각화
  ...
  {
    text: "두 ControlNet의 강도를 격자로 훑어 최적 조합을 찾아. 포즈 스케일(s1)과 뎁스 스케일(s2)을 각각 0.0→1.0으로 6단계씩, 6×6=36칸을 돌려. image=[포즈맵, 뎁스맵]에 controlnet_conditioning_scale=[s1, s2]로 둘의 비중을 따로 줘. 격자를 보면 한쪽이 0이면 그 조건이 빠지고, 둘 다 높으면 자세와 깊이를 동시에 강하게 구속하는 게 보여 — 실전에선 이렇게 스윕해서 '포즈는 충실, 깊이는 은은하게' 같은 황금비를 찾아 쓰는 거야. negative_prompt로 해부학 오류도 억눌러.",
    diagram: {
