...
const explanations: ExplanationEntry[] = [
  ...
  // 1 — SD1.5 파이프라인 로드
  {
    text: "본체 SD1.5를 float16으로 올리고 스케줄러를 DDIM 계열로 설정해(algorithm_type='sde-dpmsolver++'). 공식 문서가 얼굴 모델엔 DDIM/Euler를 권장하는 이유는, 얼굴 정체성이 매 스텝 무작위성에 흔들리면 사람이 바뀌어 보이기 때문이야 — 안정적인 샘플러로 일관성을 잡는 거지. 아직 IP-Adapter는 안 붙은 순수 SD 상태야.",
    lines: {
      4: "safety_checker=None — NSFW 필터 끔. 메모리 절약·검열 단계 생략.",
      6: "device_map=device — 모델을 GPU(또는 CPU)에 자동 배치.",
      7: "algorithm_type=\"sde-dpmsolver++\" — DDIM 설정에 SDE 변형 솔버 지정. 디테일 보존에 유리한 샘플링 방식.",
    },
  },
  ...
  // 3 — 얼굴 정체성 생성 (셰프 프롬프트)
  {
    text: "정체성 유지를 실제로 돌려봐. set_ip_adapter_scale(0.4)로 '얼굴은 적당히, 장면은 프롬프트대로'의 균형을 잡고, 프롬프트로 '프랑스 식당에서 앞치마 두르고 요리하는 셰프'를 주면서 ip_adapter_image=얼굴을 함께 넣어 5장을 뽑아. negative_prompt로 해부학 오류·저화질을 억눌러. 결과는 전부 같은 인물인데 셰프가 된 모습 — 사진 한 장으로 사람을 임의의 상황에 넣는, 개인화 아바타/프로필 생성의 핵심 동작이야.",
    lines: {
      1: "set_ip_adapter_scale(0.4) — 얼굴 영향력을 0.4로. 정체성과 장면 표현의 균형점.",
      7: "프롬프트는 텍스트 그대로 '장면(셰프)'을 지시. 얼굴은 ip_adapter_image로 따로 주입.",
      8: "ip_adapter_image=image — 이 얼굴 사진이 정체성 통로로 들어가. 텍스트와 별개 손잡이.",
      9: "negative_prompt — 피하고 싶은 것(저해상·해부학 오류) 명시. 얼굴 일그러짐 억제에 중요.",
    },
  },
  // 4 — 얼굴 scale 스윕
  {
    text: "얼굴을 얼마나 강하게 닮게 할지(set_ip_adapter_scale)를 0.1→0.9로 훑어. 낮으면 프롬프트(셰프 장면)가 지배해 인물 닮음이 옅어지고, 높이면 얼굴에 딱 붙되 장면 표현이 줄어들어. 얼굴 IP-Adapter는 보통 0.4~0.6이 단맛 구간이야 — 정체성과 연출의 균형. 시드를 0으로 고정해 scale 효과만 깨끗이 비교해.",
    lines: {
      5: "scale을 0.1~0.9로 바꿔가며 한 장씩. 정체성 강도의 전 구간을 훑어.",
      6: "루프 안에서 매번 set_ip_adapter_scale로 강도만 갱신. 비교의 핵심 변수.",
      14: "generator=torch.manual_seed(0) — 시드 고정. 노이즈를 똑같이 깔아 scale 효과만 순수 비교.",
    },
  },
  // 5 — 프롬프트 목록 정의
