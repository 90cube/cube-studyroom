...
const explanations: ExplanationEntry[] = [
  ...
  {
    text: "그림→그림 실행. img2img_pipe에 원본 image와 프롬프트 'a man with neon green hair'를 주고, 핵심 다이얼인 strength=0.2로 돌려. strength는 '원본을 얼마나 갈아엎을지'야 — 0.2면 원본에 노이즈를 살짝(20%)만 입히고 거기서부터 복원하니, 얼굴·구도는 그대로 두고 머리색 같은 디테일만 프롬프트대로 바뀌어. (내부적으론 50스텝 중 20%인 약 10스텝만 실제로 도는 거라 빠르기도 해.) strength를 키우면 원본에서 점점 멀어지고, 1에 가까우면 거의 새 그림이 돼. 원본과 결과를 나란히 띄워 변화를 확인해. 이 'strength로 보존↔변형 조절'이 실무에서 제품 사진 리터칭·색 변경·배경 교체에 그대로 쓰이는 SDEdit 방식이야.",
    diagram: {
  ...
  {
    text: "인페인트에 쓸 원본과 마스크를 준비해. 사진(dt.png)을 얼굴 위주로 crop하고 512×512로 맞춰. 그다음 마스크: 512×512 검은 판(전부 0)에서 위쪽 200줄만 1로 칠해 — 여기가 '다시 그릴 영역'(머리 부분)이야. gaussian_filter(sigma=10)로 경계를 부드럽게 번지게 해서 이음매가 티 안 나게 하고, 0~255 흑백 이미지로 변환해. 원본 위에 마스크를 반투명으로 겹쳐 띄워서 어디를 새로 칠할지 눈으로 확인해. (이 마스크가 곧 모델한테 '여기만 건드려'라고 알려주는 지도야.)",
    diagram: {
      title: "인페인트 마스크 만들기",
      kind: "algorithm",
      summary: `flowchart TD
