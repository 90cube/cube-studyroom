  ...
  cells: [
    {
      type: "markdown",
      source:
        "## 개념 — 노드는 두 겹이다\nComfyUI 노드 하나는 **두 겹**으로 돼 있어. 아래층은 **계산하는 파이썬(백엔드)** — 입력을 받아 텐서/문자열을 뱉어. 위층은 **위젯을 그리는 JS(프론트, LiteGraph)** — 화면의 노드 박스·슬라이더·버튼이야.\n핵심은 이거야: **파이썬만으로도 노드는 완성된다.** 숫자·문자·콤보·이미지 같은 기본 위젯은 파이썬이 알아서 만들어줘. 버튼·캔버스 같은 커스텀 UI가 필요할 때만 JS를 더해(그건 Part 2). 그래서 첫 노드는 파이썬 클래스 하나면 끝이야.\n\n> **'무조건 파이썬+JS만 써야 하나?'** — 아니야, 둘 다 *입구*일 뿐이야. ① 백엔드는 **파이썬이 진입점**일 뿐, `run()` 안에서 외부 API 호출·컴파일된 바이너리(Rust·C++) 실행·다른 프로세스 다 돼 — **무거운 계산은 파이썬일 필요 없어**(노드는 얇은 껍데기). ② 프론트는 브라우저라 **결국 JS로 실행**되지만, *작성*은 **바닐라 JS·TypeScript·React·Vue·Svelte 무엇이든**(framework-agnostic) — 빌드해서 JS로 올리면 돼. ComfyUI 코어 자체가 **Vue 3 + TS**고 공식 React 확장 템플릿도 있어. ③ 게다가 위 예시처럼 **대부분 노드는 프론트가 0줄**이야.",
    },
    {
      type: "markdown",
      source:
